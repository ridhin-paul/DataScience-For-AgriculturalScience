...
It is not expected to run this code unless any changes in data or more data is required, data necessary for the analysis have been collected, formatted and written into csv file under the folder name Weather data. All downloaded data has been ignored because of size constraints; if any raw data is required the specific code can be run and data be downloaded locally.
...
for (i in 1:nrow(coord))
{
    ...
    {
      wi_kab_nov_22_sr <- get_data(files_nov_22, loc, var) |>
        rename(solar_radiation_flux = Value,SRFunit = Unit)
      wi_kab_dec_23_sr <- get_data(files_dec_23, loc, var) |>
        rename(solar_radiation_flux = Value,SRFunit = Unit)
    }
    
    else if(loc$location == "Thurgem")
    {
      wi_thur_jun_22_sr <- get_data(files_june_22, loc, var) |>
        rename(solar_radiation_flux = Value,SRFunit = Unit)
      wi_thur_jul_23_sr <- get_data(files_jul_23, loc, var) |>
        rename(solar_radiation_flux = Value,SRFunit = Unit)
      wi_thur_dec_23_sr <- get_data(files_dec_23, loc, var) |>
        rename(solar_radiation_flux = Value,SRFunit = Unit)
    }
    
    else if(loc$location == "Kapsorok")
    {
      wi_kaps_jun_22_sr <- get_data(files_dec_23, loc, var) |>
        rename(solar_radiation_flux = Value,SRFunit = Unit)
    }
}
```
...
Just change the request body and change folder name required
...
for (i in 1:nrow(coord))
{
  loc <- coord[i, ] if (loc$location == "Kabianga") {
